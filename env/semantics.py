import math
from typing import (Any, Type, Callable, Union, Optional, Tuple, List, Dict, Set, NamedTuple)
from operator import attrgetter

from Box2D import (b2Vec2, b2World, b2Body, b2Shape, b2CircleShape, b2Transform)
...
class SpawnGrid(Spawner):
    ...
    def reset(self):
        ...
        self.positions = [b2Vec2(2, 2), b2Vec2(2, 2.5), b2Vec2(-2, -2), b2Vec2(-2, -2.5)]
        self.occupied = []
...
class SafeZone(sim.Module):
    ...
    def _update_shrinking_zone(self):
        ...
        self.zone = (shape, transform)"""
        # Rendi la zona nulla (raggio zero) alla fine del cooldown
        if self.t_shrink <= 0:
            self.zone = (sim.circle_shape(0), sim.transform(translation=b2Vec2(0, 0)))
        else:  # Mantieni la zona invariata durante il periodo di cooldown
            self.zone = (sim.circle_shape(self.radiuses[self.phase]), sim.transform(translation=self.centers[self.phase]))
...
class TwoTeams(sim.Module):
    ...
    def team_is_alive(self, team_id):
        return any(self.index_bodies.bodies[i] is not None for i in self.teams[team_id])
