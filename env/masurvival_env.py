...
from Box2D import (b2World, b2Body, b2Fixture, b2Joint, b2Vec2, b2PolygonShape)
from numpy import ndarray

import env.simulation as sim
from env.semantics import (SpawnGrid, ResetSpawns, agent_prototype, box_prototype, ThickRoomWalls, Health, Heal,
                           ContinuousMelee, Item, item_prototype, Inventory, AutoPickup, UseLast, DeathDrop, SafeZone,
                           BattleRoyale, GiveLast, Object, ObjectItem, ImmunityPhase, Melee, RandomizeBoxShapes,
                           OwnedObject, OwnedObjectItem, TrackKills, TwoTeams, TeamBadge, TrackUse)
...
class BaseEnv(gym.Env):
    ...
    def step(self, # type: ignore
        ...
        infos = {'termination': None, 'status': [None, None]}
        obs = self.fetch_observations(agents)
        rewards, info = self.compute_rewards(agents, **self.config['reward_scheme'])
        infos['status'] = info
        done, info = self.is_done(agents)
        infos['termination'] = info
        self.steps += 1
        self.post_step()
        return obs, rewards, done, infos # type: ignore
    ...
    def compute_rewards(self, agents: sim.Group, sparse: bool) -> [float, list[str], ...]:
        raise NotImplementedError
...
onevsone_heals_config: Config = {
    ...
    'reward_scheme': {
        ...
        # reward at death
        'r_death': -10,
    },                                                # Not sure about 'r_dead' with 'gameover':{'mode': 'lastalive'}
    'gameover': {
        'mode': 'lastalive',  # in {alldead, lastalive}
    },
    'rng': {'seed': 42},
    'spawn_grid': {
        'grid_size': 4,
    ...
    'motors': {
        'impulse': (0.75, 0.75, 0.5),
        'drift': False,
    },
    'health': {
        'health': 100,
    },                                  # Health 0 only when the agent falls from the arena
    'melee': {
        'range': 0.001,
        'damage': 0,
